...
with recursive paths as (
  ...
  select * from (
    ...
    select distinct on (e.b)
      e.b,
      array_append(visited, e.b)
    from paths p
    inner join edges e
      on e.a = p.current and e.b not in (select unnest(visited) from paths)
    order by e.b, array_length(visited, 1)
  ) q
)
select array_length(visited, 1) - 1 steps
from paths p
inner join cells c on c.id = p.current
where c.cell = 'E'
