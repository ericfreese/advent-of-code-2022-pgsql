with monkey_data as (
  ...
  select
    id[1]::int id,
    string_to_array(items[1], ', ')::int[] items,
      ...
      select * from (
        ...
        resolved_operands as (
          select
            *,
            array[
              case when operands[1] = 'old' then item else operands[1]::int end,
              case when operands[2] = 'old' then item else operands[2]::int end
            ] resolved_operands
        ...
        select
          ...
          case
            when (turn + 1) % (select count(*) from monkeys) = monkey_id then
              ...
              items || coalesce(ci.caught_items, array[]::int[])
          end
        from turns b
        left join caught_items ci using (monkey_id)
        where turn + 1 < (select count(*) from monkeys) * 20
      ) q
    )
    select * from turns
  ) q
