  ...
  with recursive sand_grains as (
    ...
    select * from (
      ...
      fixed as (
        ...
        union all
        select pos[1], pos[2] from sand_grains where not is_falling
      ),
      next as (
        select
          *,
          case
            when fb.x is null then
              array[pos[1], pos[2] + 1]
            when fl.x is null then
              array[pos[1] - 1, pos[2] + 1]
            when fr.x is null then
              array[pos[1] + 1, pos[2] + 1]
            else
              pos
          end next_pos
        from sand_grains s
        left join fixed fl on fl.y = s.pos[2] + 1 and fl.x = s.pos[1] - 1
        left join fixed fr on fr.y = s.pos[2] + 1 and fr.x = s.pos[1] + 1
        left join fixed fb on fb.y = s.pos[2] + 1 and fb.x = s.pos[1]
        where is_falling
      )
      select * from (
        select
        ...
        select
          ...
          true
        from sand_grains
        having (select count(*) from sand_grains where is_falling) = 0
      ) q
      where (select max(pos[2]) from sand_grains) <= (select max_y from bounds)
    ) q
  )
  select
    pos[1] x,
    pos[2] y
  from sand_grains
  where not is_falling and step = (select max(step) from sand_grains)
);
...
cross join generate_series((select min_y from bounds), (select max_y from bounds)) by(y)
